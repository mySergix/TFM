//------------------------------------------------------------------------------------------------//
//               CPP FILE FOR NON PREMIXED CASE SPECIES BOUNDARY CONDITIONS SETUP                 //
//------------------------------------------------------------------------------------------------//
...
int i, j, k, n;
    ...
    if (Ix[Rango] <= NX_1){
        for (i = Ix[Rango]; i < min(Fx[Rango], NX_1); i++){
            for (k = 0; k < NZ; k++){
                ...
                // Species 0 - CH4
                Species[0].Bottom[BOTTOM(i,0,k)] = 0.0;

                // Species 1 - O2
                Species[1].Bottom[BOTTOM(i,0,k)] = 0.0;
    ...
    // Inlet 2 Flow
    if (Fx[Rango] >= NX_1 + NX_2){

        for (i = max(Ix[Rango],NX_1 + NX_2); i < Fx[Rango]; i++){
            for (k = 0; k < NZ; k++){
