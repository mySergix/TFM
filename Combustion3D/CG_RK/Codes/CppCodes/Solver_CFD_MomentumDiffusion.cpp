...
int i, j, k;
    ...
    for(i = Ix[Rango]; i < Fx[Rango] + 1; i++){
        for(j = MESH.NY_ColumnMU[i + Halo - Ix[Rango]][0] + 1; j < MESH.NY_ColumnMU[i + Halo - Ix[Rango]][1] - 1; j++){
			...
			U.Diffusive[LU(i,j,k,0)] =  (1.0/(Rho*MESH.VolMU[LU(i,j,k,0)]))*(
                                         ...
                                         - 0.25 * (mu_visc[LP(i-1,j-1,k,0)] + mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i-1,j,k,0)] + mu_visc[LP(i,j,k,0)]) * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j-1,k,0)]) / MESH.DeltasMV[LV(i,j,k,1)]
										 + 0.25 * (mu_visc[LP(i-1,j,k,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i-1,j,k+1,0)] + mu_visc[LP(i,j,k+1,0)]) * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k+1,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
										 - 0.50 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i-1,j,k,0)]) * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k,0)] - U.Here[HERE(i,j,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
										 );
    ...
    for(i = Ix[Rango]; i < Fx[Rango] + 1; i++){
        for(j = MESH.NY_ColumnMU[i + Halo - Ix[Rango]][0] + 1; j < MESH.NY_ColumnMU[i + Halo - Ix[Rango]][1] - 1; j++){
			...
			U.Diffusive[LU(i,j,k,0)] =  (1.0/(Rho*MESH.VolMU[LU(i,j,k,0)]))*(
										 ...
										 + 0.25 * (mu_visc[LP(i-1,j,k,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i-1,j+1,k,0)] + mu_visc[LP(i,j+1,k,0)]) * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j+1,k,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMV[LV(i,j+1,k,1)]
                                         - 0.25 * (mu_visc[LP(i-1,j-1,k,0)] + mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i-1,j,k,0)] + mu_visc[LP(i,j,k,0)]) * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j-1,k,0)]) / MESH.DeltasMV[LV(i,j,k,1)]
										 + 0.50 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i-1,j,k,0)]) * MESH.SupMU[LU(i,j,k,2)] * (U.There[THERE(i,j,NZ)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
										 - 0.25 * (mu_visc[LP(i-1,j,k-1,0)] + mu_visc[LP(i,j,k-1,0)] + mu_visc[LP(i-1,j,k,0)] + mu_visc[LP(i,j,k,0)]) * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j,k-1,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
										 );
    ...
    if (Rango == 0){
        ...
        for(j = 1; j < NY - 1; j++){
		    for(k = 1; k < NZ - 1; k++){	
				U.Diffusive[LU(i,j,k,0)] =  (1.0/(Rho*2.0*MESH.VolMU[LU(i,j,k,0)]))*(
                                         + mu_visc[LP(i,j,k,0)] * 2.0 * MESH.SupMU[LU(i,j,k,0)] * (U_Matrix[LU(i+1,j,k,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMP[LP(i,j,k,0)]
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j+1,k,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMV[LV(i,j+1,k,1)]
                                         - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j-1,k,0)]) / MESH.DeltasMV[LV(i,j,k,1)]
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k+1,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
										 - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j,k-1,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
										 );
		...
		for(k = 1; k < NZ - 1; k++){	
			U.Diffusive[LU(i,j,k,0)] =  (1.0/(Rho*2.0*MESH.VolMU[LU(i,j,k,0)]))*(
                                         + mu_visc[LP(i,j,k,0)] * 2.0 * MESH.SupMU[LU(i,j,k,0)] * (U_Matrix[LU(i+1,j,k,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMP[LP(i,j,k,0)]
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j+1,k,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMV[LV(i,j+1,k,1)]
                                         - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j,k,0)] - U.Bottom[BOTTOM(i,0,k)]) / MESH.DeltasMV[LV(i,j,k,1)]
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k+1,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
										 - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j,k-1,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
										 );
		...
		for(k = 1; k < NZ - 1; k++){	
			U.Diffusive[LU(i,j,k,0)] =  (1.0/(Rho*2.0*MESH.VolMU[LU(i,j,k,0)]))*(
                                         + mu_visc[LP(i,j,k,0)] * 2.0 * MESH.SupMU[LU(i,j,k,0)] * (U_Matrix[LU(i+1,j,k,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMP[LP(i,j,k,0)]
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,1)] * (U.Top[TOP(i,NY,k)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMV[LV(i,j+1,k,1)]
                                         - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j-1,k,0)]) / MESH.DeltasMV[LV(i,j,k,1)]
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k+1,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
										 - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j,k-1,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
										 );
		...
		for(j = 1; j < NY - 1; j++){
			U.Diffusive[LU(i,j,k,0)] =  (1.0/(Rho*2.0*MESH.VolMU[LU(i,j,k,0)]))*(
                                         ...
                                         - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j-1,k,0)]) / MESH.DeltasMV[LV(i,j,k,1)]
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k+1,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
										 - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k,0)] - U.Here[HERE(i,j,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
										 );
		...
		for(j = 1; j < NY - 1; j++){
			U.Diffusive[LU(i,j,k,0)] =  (1.0/(Rho*2.0*MESH.VolMU[LU(i,j,k,0)]))*(
										 ...
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j+1,k,0)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMV[LV(i,j+1,k,1)]
                                         - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,1)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j-1,k,0)]) / MESH.DeltasMV[LV(i,j,k,1)]
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U.There[THERE(i,j,NZ)] - U_Matrix[LU(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
										 - mu_visc[LP(i,j,k,0)] * MESH.SupMU[LU(i,j,k,2)] * (U_Matrix[LU(i,j,k,0)] - U_Matrix[LU(i,j,k-1,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
										 );
		...
		for(j = MESH.NY_ColumnMV[i + Halo - Ix[Rango]][0] + 1; j < MESH.NY_ColumnMV[i + Halo - Ix[Rango]][1] - 1; j++){
			for(k = 1; k < NZ - 1; k++){
				...
				V.Diffusive[LV(i,j,k,0)] = (1.0/(Rho*MESH.VolMV[LV(i,j,k,0)]))*(
										 + 0.25 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i+1,j,k,0)] + mu_visc[LP(i+1,j-1,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (V_Matrix[LV(i+1,j,k,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMU[LU(i+1,j,k,0)]
                                         - 0.25 * (mu_visc[LP(i-1,j,k,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i-1,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i-1,j,k,0)]) / MESH.DeltasMU[LU(i,j,k,0)]	
				...
				if (i == NX_1 - 1 && j < NY - (NY_3 + NY_4)){
					V.Diffusive[LV(i,j,k,0)] = (1.0/(Rho*MESH.VolMV[LV(i,j,k,0)]))*(
										 + 0.50 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (0.0 - V_Matrix[LV(i,j,k,0)]) / (0.50 * MESH.DeltasMU[LU(i+1,j,k,0)])
                                         - 0.25 * (mu_visc[LP(i-1,j,k,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i-1,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i-1,j,k,0)]) / MESH.DeltasMU[LU(i,j,k,0)]	
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j+1,k,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMP[LP(i,j,k,1)]
                                         - mu_visc[LP(i,j-1,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i,j-1,k,0)]) / MESH.DeltasMP[LP(i,j-1,k,1)]
										 + 0.25 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i,j,k+1,0)] + mu_visc[LP(i,j-1,k+1,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V_Matrix[LV(i,j,k+1,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
				...
				// Internal Right
					...
					V.Diffusive[LV(i,j,k,0)] = (1.0/(Rho*MESH.VolMV[LV(i,j,k,0)]))*(
										 + 0.25 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i+1,j,k,0)] + mu_visc[LP(i+1,j-1,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (V_Matrix[LV(i+1,j,k,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMU[LU(i+1,j,k,0)]
                                         - 0.50 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (V_Matrix[LV(i,j,k,0)] - 0.0) / (0.50 * MESH.DeltasMU[LU(i,j,k,0)])	
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j+1,k,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMP[LP(i,j,k,1)]
                                         - mu_visc[LP(i,j-1,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i,j-1,k,0)]) / MESH.DeltasMP[LP(i,j-1,k,1)]
										 + 0.25 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i,j,k+1,0)] + mu_visc[LP(i,j-1,k+1,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V_Matrix[LV(i,j,k+1,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
                                         - 0.25 * (mu_visc[LP(i,j,k-1,0)] + mu_visc[LP(i,j-1,k-1,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i,j,k-1,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
    ...
    for(i = Ix[Rango]; i < Fx[Rango]; i++){
		for(j = MESH.NY_ColumnMV[i + Halo - Ix[Rango]][0] + 1; j < MESH.NY_ColumnMV[i + Halo - Ix[Rango]][1] - 1; j++){
			...
			V.Diffusive[LV(i,j,k,0)] = (1.0/(Rho*MESH.VolMV[LV(i,j,k,0)]))*(
                                         ...
                                         - mu_visc[LP(i,j-1,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i,j-1,k,0)]) / MESH.DeltasMP[LP(i,j-1,k,1)]
										 + 0.25 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i,j,k+1,0)] + mu_visc[LP(i,j-1,k+1,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V_Matrix[LV(i,j,k+1,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
                                         - 0.50 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V_Matrix[LV(i,j,k,0)] - V.Here[HERE(i,j,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
										 );
    ...
    for(i = Ix[Rango]; i < Fx[Rango]; i++){
		for(j = MESH.NY_ColumnMV[i + Halo - Ix[Rango]][0] + 1; j < MESH.NY_ColumnMV[i + Halo - Ix[Rango]][1] - 1; j++){
			...
			V.Diffusive[LV(i,j,k,0)] = (1.0/(Rho*MESH.VolMV[LV(i,j,k,0)]))*(
										 ...
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j+1,k,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMP[LP(i,j,k,1)]
                                         - mu_visc[LP(i,j-1,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i,j-1,k,0)]) / MESH.DeltasMP[LP(i,j-1,k,1)]
										 + 0.50 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V.There[THERE(i,j,NZ)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
                                         - 0.25 * (mu_visc[LP(i,j,k-1,0)] + mu_visc[LP(i,j-1,k-1,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i,j,k-1,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
										 );
			...
			if (i == NX_1 - 1 && j < NY - (NY_3 + NY_4)){
				V.Diffusive[LV(i,j,k,0)] = (1.0/(Rho*MESH.VolMV[LV(i,j,k,0)]))*(
										 + 0.25 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i+1,j,k,0)] + mu_visc[LP(i+1,j-1,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (0.0 - V_Matrix[LV(i,j,k,0)]) / (0.50 * MESH.DeltasMU[LU(i+1,j,k,0)])
                                         - 0.25 * (mu_visc[LP(i-1,j,k,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i-1,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i-1,j,k,0)]) / MESH.DeltasMU[LU(i,j,k,0)]	
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j+1,k,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMP[LP(i,j,k,1)]
                                         - mu_visc[LP(i,j-1,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i,j-1,k,0)]) / MESH.DeltasMP[LP(i,j-1,k,1)]
										 + 0.50 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V.There[THERE(i,j,NZ)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
			...
			// Internal Right
				...
				V.Diffusive[LV(i,j,k,0)] = (1.0/(Rho*MESH.VolMV[LV(i,j,k,0)]))*(
										 + 0.25 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i+1,j,k,0)] + mu_visc[LP(i+1,j-1,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (V_Matrix[LV(i+1,j,k,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMU[LU(i+1,j,k,0)]
                                         - 0.25 * (mu_visc[LP(i-1,j,k,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i-1,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,0)] * (V_Matrix[LV(i,j,k,0)] - 0.0) / (0.50 * MESH.DeltasMU[LU(i,j,k,0)])	
										 + mu_visc[LP(i,j,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j+1,k,0)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMP[LP(i,j,k,1)]
                                         - mu_visc[LP(i,j-1,k,0)] * MESH.SupMV[LV(i,j,k,1)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i,j-1,k,0)]) / MESH.DeltasMP[LP(i,j-1,k,1)]
										 + 0.50 * (mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V.There[THERE(i,j,NZ)] - V_Matrix[LV(i,j,k,0)]) / MESH.DeltasMW[LW(i,j,k+1,2)]
                                         - 0.25 * (mu_visc[LP(i,j,k-1,0)] + mu_visc[LP(i,j-1,k-1,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k,0)]) * MESH.SupMV[LV(i,j,k,2)] * (V_Matrix[LV(i,j,k,0)] - V_Matrix[LV(i,j,k-1,0)]) / MESH.DeltasMW[LW(i,j,k,2)]
		...
		for(j = MESH.NY_ColumnMW[i + Halo - Ix[Rango]][0] + 1; j < MESH.NY_ColumnMW[i + Halo - Ix[Rango]][1] - 1; j++){
			for(k = 1; k < NZ; k++){
				...
				W.Diffusive[LW(i,j,k,0)] = (1.0/(Rho*MESH.VolMW[LW(i,j,k,0)]))*(
                                         ...
                                         - 0.25 * (mu_visc[LP(i,j-1,k,0)] + mu_visc[LP(i,j,k,0)] + mu_visc[LP(i,j-1,k-1,0)] + mu_visc[LP(i,j,k-1,0)]) * MESH.SupMW[LW(i,j,k,1)] * (W_Matrix[LW(i,j,k,0)] - W_Matrix[LW(i,j-1,k,0)]) / MESH.DeltasMV[LV(i,j,k,1)]
